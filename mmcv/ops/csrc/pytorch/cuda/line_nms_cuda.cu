...
std::vector<Tensor> LineNMSForwardCudaKernelLauncher(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(boxes.scalar_type(), "line_nms_cuda_forward_cuda_kernel", ([&] {
    line_nms_cuda_forward_cuda_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
        boxes_num,
  ...
  nms_collect<<<1, min(col_blocks, THREADS_PER_BLOCK), col_blocks * sizeof(unsigned long long), stream>>>(
      boxes_num,
      col_blocks,
      top_k,
      idx.data_ptr<int64_t>(),
