...
__device__ inline bool devIoU(scalar_t const * const a, scalar_t const * const b, const scalar_t threshold) {
  const int start_a = static_cast<int>(a[2] * N_STRIPS - DATASET_OFFSET + 0.5f); // 0.5 rounding trick
  const int start_b = static_cast<int>(b[2] * N_STRIPS - DATASET_OFFSET + 0.5f);
  const int start = max(start_a, start_b);
  const int end_a = start_a + static_cast<int>(a[4]) - 1 + static_cast<int>(0.5f - ((a[4] - 1) < 0)); //  - (x<0) trick to adjust for negative numbers (in case length is 0)
  const int end_b = start_b + static_cast<int>(b[4]) - 1 + static_cast<int>(0.5f - ((b[4] - 1) < 0));
...
template <typename scalar_t>
__global__ void line_nms_cuda_forward_cuda_kernel(const int64_t n_boxes, const scalar_t nms_overlap_thresh,
  ...
  const int64_t row_size = min((n_boxes - row_start * threadsPerBlock), threadsPerBlock);
  const int64_t col_size = min((n_boxes - col_start * threadsPerBlock), threadsPerBlock);
  ...
  if (threadIdx.x < col_size) {
    ...
    if (box_idx >= 0 && box_idx < n_boxes) {  // Add bounds check
      for (int i = 0; i < PROP_SIZE; ++i) {
        shared_boxes[threadIdx.x * PROP_SIZE + i] = dev_boxes[box_idx * PROP_SIZE + i];
      }
    }
  }
  __syncthreads();
  ...
  if (threadIdx.x < row_size) {
    ...
    for (int i = start; i < col_size; i++) {
      if (devIoU<scalar_t>(cur_box, shared_boxes + i * PROP_SIZE, nms_overlap_thresh)) {
        t |= 1ULL << i;
      }
    }
...
__global__ void nms_collect(const int64_t boxes_num, const int64_t col_blocks, int64_t top_k,
                           const int64_t *idx, const int64_t *mask, int64_t *keep,
                           int64_t *parent_object_index, int64_t *num_to_keep) {
  ...
  for (int64_t i = num_to_keep_; i < boxes_num; ++i)
      keep[i] = 0;
